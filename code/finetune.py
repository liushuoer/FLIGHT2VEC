...
from src.learner import Learner, Flight2VecLoss
from src.callback.core import Flight2VecCB

import argparse

patch_len = 32
path = "/Users/shuoliu/Downloads/pathformer-main/Flight2Vec/code/saved_models/aircraft/masked_patchtst/based_model/aircraft_patchtst_finetuned_cw128_tw15_patch32_stride32_epochs-finetune4_model2.pth"
...
parser.add_argument('--dset_finetune', type=str, default='aircraft', help='dataset name')
parser.add_argument('--context_points', type=int, default=128, help='sequence length')
parser.add_argument('--target_points', type=int, default=3, help='forecast horizon')
...
parser.add_argument('--stride', type=int, default=patch_len, help='stride between patch')
# RevIN
...
# Optimization args
parser.add_argument('--n_epochs_finetune', type=int, default=4, help='number of finetuning epochs')
parser.add_argument('--lr', type=float, default=1e-4, help='learning rate')
# Pretrained model name
parser.add_argument('--pretrained_model', type=str, default=path, help='pretrained model name')
# parser.add_argument('--pretrained_model', type=str, default=None, help='pretrained model name')

# model id to keep track of the number of models saved
parser.add_argument('--finetuned_model_id', type=int, default=2, help='id of the saved finetuned model')
...
def find_lr(head_type):
    ...
    # get loss
    loss_func = Flight2VecLoss(lambda_weight=1.0)
    # get callbacks
    cbs = [RevInCB(dls.vars)] if args.revin else []
    cbs += [PatchCB(patch_len=args.patch_len, stride=args.stride)]
    cbs += [Flight2VecCB(patch_len=args.patch_len)]
...
def finetune_func(lr=args.lr):
    ...
    cbs += [
        PatchCB(patch_len=args.patch_len, stride=args.stride,
                behavior_adaptive=True, angle_threshold=15.0),  # Enable behavior-adaptive
        Flight2VecCB(patch_len=args.patch_len),
...
def test_func(weight_path):
    ...
    print('Inference time for one sequence:', inference_time_ms, 'ms')
    print('length:',len(out[0]))         # out: a list of [pred, targ, score]
    print('score:', out[2])
    # save results
