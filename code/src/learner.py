...
class Flight2VecLoss(nn.Module):
    ...
    def forward(self, pred, target):
        """
        pred: (reconstruction_pred, direction_logits)
        target: (reconstruction_target, direction_labels)
        """
        reconstruction_pred, direction_logits = pred
        reconstruction_target, direction_labels = target

        # MSE loss for reconstruction
        l_mse = self.mse_loss(reconstruction_pred, reconstruction_target)

        # Cross-entropy loss for direction classification
        l_md = self.direction_loss(direction_logits, direction_labels)

        # Combined loss: L_m = L_MD + λ·L_MSE
        total_loss = l_md + self.lambda_weight * l_mse
...
class Learner(GetAttr):
    ...
    def train_step(self, batch):
        if len(batch) == 2 and isinstance(batch[1], tuple):
            # Flight2Vec case: batch = (xb, (yb, direction_labels))
            self.xb, (self.yb, self.direction_labels) = batch
            # Store direction labels for later use
            self._direction_labels = self.direction_labels
        else:
            # Normal case: batch = (xb, yb)
            self.xb, self.yb = batch
            self._direction_labels = None

        pred = self.model_forward()

        # Check for Flight2Vec multi-output case
        if isinstance(pred, tuple) and len(pred) == 2:
            if self._direction_labels is not None:
                # Flight2Vec case with direction labels
                loss = self.loss_func(pred, (self.yb, self._direction_labels))
            else:
                # Fallback to MSE if no direction labels
                reconstruction_pred = pred[0]
                loss = torch.nn.functional.mse_loss(reconstruction_pred, self.yb)
        else:
