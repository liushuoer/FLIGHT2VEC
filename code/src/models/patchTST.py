...
class PatchTST(nn.Module):
    ...
    def __init__(self, c_in:int, target_dim:int, patch_len:int, stride:int, num_patch:int, 
        ...
        if head_type == "pretrain":
            ...
            # self.head = PredictionHead(individual, self.n_vars, d_model, num_patch, num_patch*patch_len, head_dropout)
        ...
        self.direction_head = ClassificationHead(self.n_vars, d_model, num_class, head_dropout)
    ...
    def forward(self, z):                             
        ...
        # print(f"self.head: {self.head}")  # Print the structure of self.head
        main_output = self.head(z)  # Main task output (prediction, regression, etc.)
        direction_logits = self.direction_head(z)  # Direction classification logits
        ...
        #    [bs x num_patch x n_vars x patch_len] for pretrain
        # Combine the outputs
        pred = (main_output, direction_logits)
        return pred
